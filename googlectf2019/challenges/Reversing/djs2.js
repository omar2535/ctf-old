...
(T.e0 = function(a, b) {
  ...
  (T.f0 = function(a) {
    var b,
      c = new Array(Math.ceil(a.length / 4));
    for (b = 0; b < c.length; b++)
      c[b] =
        a.charCodeAt(4 * b) +
        (a.charCodeAt(4 * b + 1) << 8) +
        (a.charCodeAt(4 * b + 2) << 16) +
        (a.charCodeAt(4 * b + 3) << 24);
    return c;
  }),
...
let choose = myArray => myArray[Math.floor(Math.random() * myArray.length)];
let generateKey = () => {
  // let example = navigator.platform.toUpperCase().substr(0, 5)
  //     + Number(/android/i.test(navigator.userAgent))
  //     + Number(/AdsBot/i.test(navigator.userAgent))
  //     + Number(/Google/i.test(navigator.userAgent))
  //     + Number(/geoedge/i.test(navigator.userAgent))
  ...
  //     + navigator.appCodeName.toUpperCase().substr(0, 7)
  //     + Number(navigator.maxTouchPoints > 0)
  //     + Number((undefined == window.chrome) ? true : (undefined == window.chrome.app)) + navigator.plugins.length
  let a =
    choose(["android", "linux"])
      .toUpperCase()
      .substr(0, 5) +
    choose(["1", "0"]) +
    choose(["1", "0"]) +
