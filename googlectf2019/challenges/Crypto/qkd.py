...
def measure(rx_qubits, basis):
  ...
  for q, b in zip(rx_qubits, basis):
    ...
    measured_bits += str(numpy.random.choice(numpy.arange(0, 2),
...
def perform(rx_qubits, rx_basis):
  ...
  sat_basis = [random.choice(
      '+x') for _ in range(len(current_app.config['ENCRYPTION_KEY'])*16)]
  measured_bits = measure(unmarshal(rx_qubits), sat_basis)
  binary_key, err = compare_bases_and_generate_key(
      rx_basis, sat_basis, measured_bits)
  if err:
